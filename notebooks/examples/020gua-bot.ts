...
function generateGua(): string[] {
  ...
  const firstGua = guaDict[firstGuaYinYang.join("") as GuaKey];
  ...
  const secondGua = guaDict[secondGuaYinYang.join("") as GuaKey];
...
你的语言应该具有仙风道骨、雅致高贵的气质，以此来展现你的卜卦专家身份。`,
  ...
  async function chat() {
    const input = (await question("User: ")) as unknown as string;
